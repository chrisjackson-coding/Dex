...
* **Setup is now all-or-nothing.** Onboarding and room changes reject links or malformed paths before touching the vault, refuse to continue when the lifecycle authority cannot run, and roll back every profile, folder, seed, configuration, and skill change if a later step fails. Their receipts name the exact files that survived the transaction; a failed, fully rolled-back run reports no successful mutations.
