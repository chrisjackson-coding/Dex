...
<!-- Generated from `.claude/skills/feedback/SKILL.md` by `scripts/generate-agents-skills.py`. Do not edit. -->
...
0. **Preflight the connection first** — before investigating or drafting anything:
   ...
   ```bash
   python3 .agents/skills/feedback/scripts/feedback_client.py check --vault "$VAULT_PATH"
   ```
...
6. **Send:**
   ...
   ```bash
   python3 .agents/skills/feedback/scripts/feedback_client.py report --file /tmp/dex-feedback-draft.json --vault "$VAULT_PATH"
   ```
...
```bash
python3 .agents/skills/feedback/scripts/feedback_client.py status --vault "$VAULT_PATH"
```
...
4. Send:
   ...
   ```bash
   python3 .agents/skills/feedback/scripts/feedback_client.py answer --ticket DEX-158 --text "<approved answer>" --vault "$VAULT_PATH"
   ```
