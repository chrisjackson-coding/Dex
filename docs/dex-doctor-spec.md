...
| calendar.access | If `calendar.provider` is `google`, the Google Calendar `calendarList` path used by Google Workspace (not EventKit). Otherwise the `calendar_list_calendars` path via EventKit; configured `work_calendar` present in the list | provider is `none`, or Apple permission never granted AND feature unused | Google Workspace connector missing (T3: `/google-workspace-setup`) / Google authorization failed (T3: `/google-workspace-setup`) / Apple permission denied (T3) / configured calendar not found (T3: set `calendar.work_calendar`, list real names) |
