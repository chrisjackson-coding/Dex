...
| career.evidence_capture | Career room on, and `settings.json` PostToolUse watches Write and Edit for `career-evidence-capture.cjs`; recent skip-log reasons are named without filenames | Career room off | hook missing, or matcher does not cover both Write and Edit (T2) |
