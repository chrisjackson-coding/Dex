...
def build_artifacts(vault: Path, workdir: Path, stamp: str) -> list[Path]:
    """Build the archive, the verified git bundle, and the checksum sidecar."""
    archive = workdir / f"{PREFIX}{stamp}.tar.gz"
    with tarfile.open(archive, "w:gz") as tar:
        tar.add(vault, arcname=ARCNAME, filter=_tar_filter)
...
class FolderBackend:
    ...
    def store(self, artifacts: list[Path]) -> str:
        for artifact in artifacts:
            ...
            partial.write_bytes(artifact.read_bytes())
            partial.rename(self.dest / artifact.name)
...
def run_backup(vault: Path) -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            artifacts = build_artifacts(vault, Path(tmp), stamp)
