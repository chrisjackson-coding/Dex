...
def test_doctor_report_is_normalized_across_the_complete_registry():
    ...
    # 38: career evidence capture and Claude project memory backup both joined
    # the quick registry in this cut. This literal is a deliberate tripwire,
    # and parallel branches can each bump it to the same stale value while
    # merging cleanly — so it is set from the complete registry's measured
    # size, not from either branch's guess.
