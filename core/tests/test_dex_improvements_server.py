...
def test_parse_session_learnings_uses_shared_pending_entries(
    backlog_file: Path,
    tmp_path: Path,
):
    directory = tmp_path / "System" / "Session_Learnings"
    directory.mkdir(parents=True, exist_ok=True)
    older = directory / "2026-06-01.md"
    newer = directory / "2026-08-01.md"
    older.write_text(
        "# Session Learnings - 2026-06-01\n\n"
        "## [14:32] - Guard failed open\n\n"
    ...
    newer.write_text(
        ...
        "## [09:00] - Already routed\n\n"
        "**What happened:** The rule already lives in the skill.\n"
        "**Why it matters:** Installing it again would duplicate the step.\n"
        "**Suggested fix:** Leave the skill as it is.\n"
        "**Status:** implemented (2026-08-02 — skill)\n\n"
        "---\n",
        encoding="utf-8",
    )

    parsed = improvements.parse_session_learnings()
    since = improvements.parse_session_learnings(since_date="2026-07-01")

    assert [item["title"] for item in parsed] == ["Guard failed open"]
    assert parsed[0]["date"] == "2026-06-01"
    assert parsed[0]["file"] == str(older)
    assert since == []
